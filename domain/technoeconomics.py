...
class TechnoEconomics:
    ...
    def compute(self, ts: pd.DataFrame) -> dict[str, float]:
        ...
        annual_h2_t = float(ts["h2_produced_kg_per_h"].sum() * self.case.time_step_h / KG_PER_T)
        annual_power_mwh = float(ts["power_to_electrolyzer_mw"].sum() * self.case.time_step_h)
        electrolyzer_capex = self.case.electrolyzer.nominal_power_mw * 1000.0 * self.case.electrolyzer.capex_usd_per_kw
        storage_capex = self.case.storage.usable_capacity_kg_h2 * self.case.storage.capex_usd_per_kg_h2 if self.case.storage.enabled else 0.0
        ...
        fixed_opex = electrolyzer_capex * self.case.electrolyzer.fixed_opex_fraction * self.case.economic.opex_multiplier
        ...
        npv = sum((methanol_revenue - annual_opex) / ((1 + self.case.economic.discount_rate) ** year) for year in range(1, self.case.economic.project_years + 1)) - total_capex
        return {
