...
class SimulationEngine:
    ...
    def run(self, case: CaseInputs) -> SimulationArtifacts:
        ...
        surrogates = MultiSurrogateManager(self.project_root, case.ptmeoh.surrogate_library)
        rows: list[dict] = []
        ...
        for _, row in case.renewable_profile.iterrows():
            ...
            if case.ptmeoh.operating_mode == "quasi_base_load":
                target_h2 = min(case.ptmeoh.target_h2_feed_kg_per_h, case.ptmeoh.max_h2_feed_kg_per_h)
                ...
                raw_available = min(el_state.h2_produced_kg_per_h, case.ptmeoh.max_h2_feed_kg_per_h)
                target_h2 = min(case.ptmeoh.target_h2_feed_kg_per_h, case.ptmeoh.max_h2_feed_kg_per_h)
            ...
            methanol_t_per_h = max(actual_h2 / KG_PER_T * case.ptmeoh.methanol_yield_t_meoh_per_t_h2, 0.0)
            if "Model_Prod_MeOH" in model_summary_df["model_name"].tolist():
                prod_row = model_summary_df.loc[model_summary_df["model_name"] == "Model_Prod_MeOH"]
                if not prod_row.empty:
                    methanol_t_per_h = max(float(prod_row["prediction"].iloc), 0.0)
            aux_power_from_models_w = float(model_summary_df.loc[model_summary_df["model_name"].str.contains("Model_Power_", regex=False), "prediction"].clip(lower=0).sum()) if not model_summary_df.empty else 0.0
            ...
            record = {
                ...
                "surrogate_all_models_in_domain": int(bool(surrogate_outputs["all_models_in_domain"])),
                "surrogate_runtime_models_count": int(surrogate_outputs["runtime_models_count"]),
                "surrogate_total_models_count": int(surrogate_outputs["total_models_count"]),
            }
        ...
        kpis = {
            ...
            "electrolyzer_full_load_hours_h": float(df["power_to_electrolyzer_mw"].sum() * case.time_step_h / max(case.electrolyzer.nominal_power_mw, 1e-9)),
            "ptmeoh_utilization_factor": float(df["h2_to_ptmeoh_kg_per_h"].mean() / max(case.ptmeoh.max_h2_feed_kg_per_h, 1e-9)),
            "renewable_utilization_fraction": float(df["power_to_electrolyzer_mw"].sum() / max(df["renewable_power_mw"].sum(), 1e-9)),
            "h2_not_supplied_t": float(df["unmet_h2_kg_per_h"].sum() * case.time_step_h / KG_PER_T),
            "curtailment_fraction": float(df["curtailed_power_mw"].sum() / max(df["renewable_power_mw"].sum(), 1e-9)),
            ...
            "runtime_models_fraction": float(df["surrogate_runtime_models_count"].max() / max(df["surrogate_total_models_count"].max(), 1)),
        }

        unmet_fraction = float(df["unmet_h2_kg_per_h"].sum() / max(df["h2_to_ptmeoh_kg_per_h"].sum() + df["unmet_h2_kg_per_h"].sum(), 1e-9))
        ...
        return SimulationArtifacts(time_series=df, kpis=kpis, warnings=warnings, surrogate_info=surrogate_info, model_summary=model_summary_df)
