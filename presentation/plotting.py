...
def heatmap(results: pd.DataFrame, z_col: str = "lcomeoh_usd_per_t_meoh"):
    ...
    fig = px.imshow(
        pivot,
        aspect="auto",
        color_continuous_scale=["#d9f0ef", "#8dd3d2", "#0b7285"],
        labels=dict(color=z_col),
    )
    fig.update_layout(
        title=f"Design heatmap — {z_col}",
        paper_bgcolor=PALETTE["bg"],
        plot_bgcolor="white",
    )
...
def tornado(sens: pd.DataFrame):
    ...
    fig = px.bar(
        data,
        x="impact_usd_per_t_meoh",
        y="parameter",
        orientation="h",
        color_discrete_sequence=[PALETTE["green"]],
    )
    fig.update_layout(
        title="Sensitivity tornado — LCOMeOH impact",
        paper_bgcolor=PALETTE["bg"],
        plot_bgcolor="white",
        xaxis_title="Impact on LCOMeOH [USD/t MeOH]",
